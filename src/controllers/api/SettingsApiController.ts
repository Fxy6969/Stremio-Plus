...
export function setupPluginSettingsAPI() {
    ...
    ipcMain.on(ENHANCED_PLUGINS_API.GET_SETTINGS_SYNC, (event, pluginFileName) => {
        try {
            const configPath = `${Properties.pluginsPath}//${pluginFileName}${FILE_EXTENSIONS.PLUGIN_CONFIG}`;
            if (!existsSync(configPath)) { event.returnValue = {}; return; }
            event.returnValue = JSON.parse(readFileSync(configPath, 'utf-8')) ?? {};
        } catch {
            event.returnValue = {};
        }
    });
