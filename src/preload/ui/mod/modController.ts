...
import { STORAGE_KEYS, SELECTORS, CLASSES, FILE_EXTENSIONS, IPC_CHANNELS } from "../../../constants";
import { settingsAPI } from "../../api/settings";
import { pluginOptionsModal } from "../../../ui/components/plugin-settings-modal/pluginSettingsModal";
...
export const modController = {
    loadPlugin: (pluginName: string): void => {
        ...
        const savedSettings: Record<string, unknown> = settingsAPI.getSettingsSync(pluginBaseName);
        ...
        (function() {
            ...
            const __stSaved__ = ${JSON.stringify(savedSettings)};
            const StremioSettings = {
                for(name) {
                    return {
                        toggle(key, label, defaultVal) {
                            const v = __stSaved__[key];
                            return () => (v !== undefined && v !== null) ? String(v) === 'true' : defaultVal;
                        },
                        color(key, label, defaultVal) {
                            const v = __stSaved__[key];
                            return () => (v !== undefined && v !== null) ? String(v) : defaultVal;
                        },
                        slider(key, label, defaultVal, opts) {
                            const v = __stSaved__[key];
                            return () => (v !== undefined && v !== null) ? Number(v) : defaultVal;
                        },
                        text(key, label, defaultVal) {
                            const v = __stSaved__[key];
                            return () => (v !== undefined && v !== null) ? String(v) : defaultVal;
                        },
                        enum(key, label, defaultVal, options) {
                            const v = __stSaved__[key];
                            return () => (v !== undefined && v !== null) ? String(v) : defaultVal;
                        }
                    };
            ...
            try {
                ...
                (function() {
                    const wk = 'sxp_errors_${pluginBaseName}';
                    const lc = 'sxp_lastcrash_${pluginBaseName}';
                    try {
                        const times = JSON.parse(localStorage.getItem(wk) || '[]');
                        times.push(Date.now());
                        const cutoff = Date.now() - 604800000;
                        const recent = times.filter(function(t) { return t > cutoff; });
                        localStorage.setItem(wk, JSON.stringify(recent));
                        localStorage.setItem(lc, String(Date.now()));
                        const errEl = document.getElementById('${pluginName}-errors');
                        if (errEl) errEl.textContent = String(recent.length);
                        const crashEl = document.getElementById('${pluginName}-lastcrash');
                        if (crashEl) crashEl.textContent = 'just now';
                    } catch(_) {}
                })();
            }
        })();
        ...
        localStorage.setItem(`sxp_coldstart_${pluginBaseName}`, String(coldStartMs));
        const coldEl = document.getElementById(`${pluginName}-coldstart`);
        if (coldEl) coldEl.textContent = `${coldStartMs}ms`;
        ...
        helpers.waitForElm(SELECTORS.PLUGINS_CATEGORY).then(() => {
            ...
            for (let i = 0; i < pluginCheckboxes.length; i++) {
                pluginCheckboxes[i].addEventListener("click", () => {
                    ...
                    const nowEnabled = pluginCheckboxes[i].classList.contains(CLASSES.CHECKED);
                    if (nowEnabled) {
                    ...
                    // Sync to enabled_plugins.json so file-based state stays consistent
                    try {
                        const meta = ExtractMetaData.extractMetadataFromFile(
                            join(Properties.pluginsPath, pluginName)
                        );
                        if (meta) {
                            ipcRenderer.invoke(IPC_CHANNELS.SET_PLUGIN_ENABLED, { name: meta.name, enabled: nowEnabled });
                        }
                    } catch(_) {}
                });
            }
    ...
    bindPluginOptionsListeners: (): void => {
        helpers.waitForElm(SELECTORS.PLUGINS_CATEGORY).then(() => {
            ...
            container.addEventListener('click', async (e) => {
                ...
                const chevron = target.closest('.sxp-chevron');
                if (chevron) {
                    chevron.closest('.sxp-plugin-card')?.classList.toggle('sxp-expanded');
                    return;
                }
